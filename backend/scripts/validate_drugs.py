...
def validate_drugs_dataset(limit: int = 1000):
    service = OptimusService()
    print(f"--- Iniciando validación de {limit} drogas ---")
    ...
    # Leer drogas usando Polars
    lf_drugs = pl.scan_parquet(path_nodes).filter(pl.col("label") == "DRG").head(limit)
    df_genes = lf_drugs.collect()
    ...
    for i, row in enumerate(df_genes.to_dicts()):
        try:
            ...
            print(f"ERROR en droga {row['id']}: {e}")
            break 
    ...
    if errors:
        ...
        print("\n¡Felicidades! Todas las drogas cumplen con el esquema estricto.")

if __name__ == "__main__":
    validate_drugs_dataset(1000)
