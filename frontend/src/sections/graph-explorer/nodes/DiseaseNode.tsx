...
export function DiseaseNode({ data, selected }: NodeProps) {
  return (
    <div className="relative flex flex-col items-center group">
      ...
      <div
        className={`flex items-center justify-center w-18 h-18 rounded-full border-2 transition-all duration-300 bg-card/90 backdrop-blur-md shadow-md ${
          selected
            ? 'border-emerald-500 text-emerald-500 shadow-[0_0_25px_rgba(16,185,129,0.5)] scale-110'
            : 'border-emerald-500/30 hover:border-emerald-500 hover:scale-105 text-emerald-500/80 hover:shadow-lg'
        }`}
      >
        {/* Subtle inner radial gradient for 3D sphere feel */}
        <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-emerald-500/10 to-transparent pointer-events-none" />
        <Heartbeat className="w-6 h-6 relative z-10" />
      </div>
      ...
      <div
        className={`mt-2 px-2 py-0.5 rounded-md text-[10px] font-bold tracking-wide uppercase transition-all duration-200 border bg-background/80 backdrop-blur-sm pointer-events-none select-none ${
          selected
            ? 'border-emerald-500/30 text-emerald-500 bg-emerald-500/5 scale-105 shadow-sm'
            : 'border-border/50 text-muted-foreground'
        }`}
      >
        {typeof data.label === 'string' ? data.label : 'DISEASE'}
      </div>
    </div>
  );
}
